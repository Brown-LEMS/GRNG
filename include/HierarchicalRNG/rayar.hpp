...
// Implementation of ApproximateRNG by Rayar et. al
...
void recursive_neighbors(unsigned int const index, int L, std::vector<tsl::sparse_set<unsigned int>>& RNG_neighbors,
                                                        LinksMap& L_links) {
    tsl::sparse_set<unsigned int>& index_neighbors = RNG_neighbors[index];
    L -= 1;

    // add all neighbors of order L
    tsl::sparse_set<unsigned int>::const_iterator it1;
    for (it1 = index_neighbors.begin(); it1 != index_neighbors.end(); it1++) {
        unsigned int const neighbor = (*it1);

        // add edge
        L_links.checkAndAddLink(index, neighbor);

        // recursively retrieve neighbors
        if (L > 0) {
            recursive_neighbors(neighbor, L, RNG_neighbors, L_links);
        }
    }
...
void getLOrderNeighbors(unsigned int const queryIndex, int L, std::vector<tsl::sparse_set<unsigned int>>& RNG_neighbors,
                        tsl::sparse_map<unsigned int, tsl::sparse_set<unsigned int>>& L_links_map) {    
    LinksMap L_links;
    L_links.clear();
    recursive_neighbors(queryIndex, L, RNG_neighbors, L_links);
    L_links_map = L_links.map;

    tsl::sparse_set<unsigned int>::const_iterator it1;
    for (it1 = RNG_neighbors[queryIndex].begin(); it1 != RNG_neighbors[queryIndex].end(); it1++) {
        unsigned int const queryNeighbor = (*it1);
        L_links_map[queryNeighbor].erase(queryIndex);
    }
}

// remove RNG links invalidated by the addition of Q
void removeInvalidatedLinks(unsigned int const queryIndex,
                            tsl::sparse_map<unsigned int, tsl::sparse_set<unsigned int>> const& L_links_map, SparseMatrix& sparseMatrix,
                            std::vector<tsl::sparse_set<unsigned int>>& RNG_neighbors) {

    tsl::sparse_map<unsigned int, tsl::sparse_set<unsigned int>>::const_iterator it1;
    for (it1 = L_links_map.begin(); it1 != L_links_map.end(); it1++) {
        unsigned int const index1 = (*it1).first;
        float const distance13 = sparseMatrix._computeDistance(index1,queryIndex);

        tsl::sparse_set<unsigned int> const& index1_neighbors = (*it1).second;
...
/**
 * @brief Construct APproximate RNG by Rayar
 * 
 */
void approximateRNG(float* const dataPointer, unsigned int const datasetSize, unsigned int const dimension, float epsilon, int L, int numThreads,
    ...
    for (queryIndex = 1; queryIndex < datasetSize; queryIndex++) {  // start on 1 since 0 doesn't give edges
        ...
        // get L-order neighbors
        tsl::sparse_map<unsigned int, tsl::sparse_set<unsigned int>> L_links_map{};
        getLOrderNeighbors(queryIndex, L, RNG_neighbors, L_links_map);

        // naive invalidation of links by indices in SR_region
        removeInvalidatedLinks(queryIndex, L_links_map, sparseMatrix, RNG_neighbors);
...
/**
 * @brief find RNG neighbors of all Q in testset by Rayar approximate rng algo
 * 
 * @param dimension 
 * @param dataPointer 
 * @param datasetSize 
...
void search(unsigned int const dimension, float* const dataPointer, unsigned int const datasetSize, float* const testPointer,
            ...
            std::vector<tsl::sparse_set<unsigned int>>& searchNeighbors, double& distanceComputations, float& time) {
    std::chrono::high_resolution_clock::time_point tStart = std::chrono::high_resolution_clock::now();
    ...
    distanceComputations = (double) (sparseMatrix.get_distanceComputationCount() / testsetSize);
    return;
}
