...
bool checkLinkForInterference_multithreading(QueryStructOfflineBuild& queryStruct, unsigned int const activeChildIndex, int const finerLayerIndex,
                                            std::shared_ptr<PivotLayerHierarchy> const& pivotLayers, SparseMatrix& sparseMatrix,
                                            SparseMatrixLocal& localSparseMatrix);
void p1_collectCandidateCoarseIndices_multithreading(unsigned int const queryIndex, int const layerIndex,
                                                    ...
                                                    SparseMatrixLocal& localSparseMatrix);
bool p2_dmaxCheck_multithreading(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
                                ...
                                SparseMatrixLocal& localSparseMatrix);
void p3_collectCandidateFineIndices_multithreading(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
                                                  ...
                                                  SparseMatrixLocal& localSparseMatrix);
bool p3a_assistedChildCheck_multithreading(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
...
bool p4_bruteForceInterferenceCheck_multithreading(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
 ...
 * @param queryStruct
 */
void OfflineBuild::stage6_multithreading(QueryStructOfflineBuild& queryStruct) {
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)activeChildrenListVector.size(); i++) {
            ...
            bool flag_interference = checkLinkForInterference_multithreading(queryStruct, activeChildIndex, finerLayerIndex, _pivotLayers,
...
bool checkLinkForInterference_multithreading(QueryStructOfflineBuild& queryStruct, unsigned int const activeChildIndex, int const finerLayerIndex,
    ...
    for (int layerIndex = 0; layerIndex < finerLayerIndex; layerIndex++) {
        ...
        if (layerIndex == 0) {
            ...
            p1_collectCandidateCoarseIndices_multithreading(queryIndex, layerIndex, pivotLayers, interferingPivotsMap[layerIndex - 1],
        ...
        flag_interference =
            p2_dmaxCheck_multithreading(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance, layerIndex, finerLayerIndex,
    ...
    p3_collectCandidateFineIndices_multithreading(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance, finerLayerIndex,
    ...
    //------------------------------------------------------------------------------
    flag_interference = p4_bruteForceInterferenceCheck_multithreading(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance,
...
void p3_collectCandidateFineIndices_multithreading(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
    ...
    for (it1 = coarseInterferingPivotsList.begin(); it1 != coarseInterferingPivotsList.end(); it1++) {
        ...
        for (it2 = coarseInterferingChildrenList.begin(); it2 != coarseInterferingChildrenList.end(); it2++) {
            ...
            flag_add = p3a_assistedChildCheck_multithreading(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance,
...
bool checkLinkForInterference(QueryStructOfflineBuild& queryStruct, unsigned int const activeChildIndex, int const finerLayerIndex,
                              std::shared_ptr<PivotLayerHierarchy> const& pivotLayers);

void p1_collectCandidateCoarseIndices(unsigned int const queryIndex, int const layerIndex, std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
                                      tsl::sparse_set<unsigned int> const& activeInterferingPivotsList, RankOrderedList& rankedCandidateList);

bool p2_dmaxCheck(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
                  float const activeChildLinkDistance, int const layerIndex, int const finerLayerIndex,
                  std::shared_ptr<PivotLayerHierarchy> const& pivotLayers, RankOrderedList const& candidateInterferingPivots,
                  tsl::sparse_set<unsigned int>& activeInterferingList);

void p3_collectCandidateFineIndices(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
                                    float const activeChildLinkDistance, int const finerLayerIndex,
                                    std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
...
bool p3a_assistedChildCheck(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
                            float const activeChildLinkDistance, unsigned int const coarseInterferingPivot, unsigned int const fineInterferingPivot,
                            std::shared_ptr<PivotLayerHierarchy> const& pivotLayers);

bool p4_bruteForceInterferenceCheck(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
                                    float const activeChildLinkDistance, tsl::sparse_set<unsigned int> const& interferingPivotsList,
                                    std::shared_ptr<PivotLayerHierarchy> const& pivotLayers);
 ...
 * @param queryStruct
 */
void OfflineBuild::stage6(QueryStructOfflineBuild& queryStruct) {
    std::chrono::high_resolution_clock::time_point tStart_stage = std::chrono::high_resolution_clock::now();
    unsigned long long int const dStart_stage = get_distanceComputationCount();
    ...
    for (it1 = activeChildrenList.begin(); it1 != activeChildrenList.end(); it1++) {
        ...
        // exhaustive, top-down search for interference
        bool flag_interference = checkLinkForInterference(queryStruct, activeChildIndex, finerLayerIndex, _pivotLayers);
        if (!flag_interference) {
...
bool checkLinkForInterference(QueryStructOfflineBuild& queryStruct, unsigned int const activeChildIndex, int const finerLayerIndex,
    ...
    for (int layerIndex = 0; layerIndex < finerLayerIndex; layerIndex++) {
        ...
        if (layerIndex == 0) {
            ...
            p1_collectCandidateCoarseIndices(queryIndex, layerIndex, pivotLayers, interferingPivotsMap[layerIndex - 1], candidateInterferingPivots);
        }

        // create list of actively interfering pivots from the candidate interfering using dmax
        // return true if any pivots found to be interfering
        flag_interference = p2_dmaxCheck(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance, layerIndex, finerLayerIndex,
    ...
    //------------------------------------------------------------------------------
    tsl::sparse_set<unsigned int> fineInterferingPivotsList;
    p3_collectCandidateFineIndices(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance, finerLayerIndex, pivotLayers,
                                   interferingPivotsMap[finerLayerIndex - 1], fineInterferingPivotsList);

    //------------------------------------------------------------------------------
    // Finally, brute force check for interference
    //------------------------------------------------------------------------------
    flag_interference = p4_bruteForceInterferenceCheck(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance,
...
void p3_collectCandidateFineIndices(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
    ...
    for (it1 = coarseInterferingPivotsList.begin(); it1 != coarseInterferingPivotsList.end(); it1++) {
        ...
        for (it2 = coarseInterferingChildrenList.begin(); it2 != coarseInterferingChildrenList.end(); it2++) {
            ...
            // assisted parent-child check
            flag_add = p3a_assistedChildCheck(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance, coarseInterferingIndex,
