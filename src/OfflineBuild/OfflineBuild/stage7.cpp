...
void p1_collectTopLayerActiveViablePivots(int pivotLayerIndex, std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
                                          RankOrderedList const& rankOrderedTopLayerPivotsList,
                                          tsl::sparse_set<unsigned int>& activeViablePivotsList);
void p2_collectActiveViablePivots(unsigned int const queryIndex, int const pivotLayerIndex, int const layerIndex,
...
void p3_collectActiveViableFinePivots(unsigned int const queryIndex, int const pivotLayerIndex,
                                      std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
                                      tsl::sparse_set<unsigned int> const& activeViableParentsList,
                                      tsl::sparse_set<unsigned int>& activeViableChildrenList);
void p4_checkFinePivotsForInterference(unsigned int const queryIndex, int const pivotLayerIndex,
                                       std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
                                       tsl::sparse_set<unsigned int> const& activeViablePivotsList, LinksMap& invalidatedLinks);
void p4_checkFinePivotForInterference_multithreading(unsigned int const queryIndex, unsigned int const activePivotIndex, int const pivotLayerIndex,
 ...
 * @param queryStruct
 */
void OfflineBuild::stage7_multithreading(QueryStructOfflineBuild& queryStruct) {
    ...
    for (int layerIndex = 0; layerIndex < pivotLayerIndex; layerIndex++) {
        ...
        if (layerIndex == 0) {
            p1_collectTopLayerActiveViablePivots(pivotLayerIndex, _pivotLayers, rankOrderedTopLayerPivotsList, activeViablePivotsListMap[0]);
        } else {
            p2_collectActiveViablePivots(queryIndex, pivotLayerIndex, layerIndex, _pivotLayers, activeViablePivotsListMap[layerIndex - 1],
                                         activeViablePivotsListMap[layerIndex]);
        }
    }
    ...
    p3_collectActiveViableFinePivots(queryIndex, pivotLayerIndex, _pivotLayers, activeViablePivotsListMap[pivotLayerIndex - 1],
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)activeViablePivotsListVector.size(); i++) {
            ...
            p4_checkFinePivotForInterference_multithreading(queryIndex, activePivotIndex, pivotLayerIndex, _pivotLayers, localInvalidatedLinks,
 ...
 * @param queryStruct
 */
void OfflineBuild::stage7(QueryStructOfflineBuild& queryStruct) {
    std::chrono::high_resolution_clock::time_point tStart_stage = std::chrono::high_resolution_clock::now();
    unsigned long long int const dStart_stage = get_distanceComputationCount();
    ...
    p4_checkFinePivotsForInterference(queryIndex, pivotLayerIndex, _pivotLayers, activeViablePivotsListMap[pivotLayerIndex], invalidatedLinks);
    queryStruct.invalidatedPivotLinks = invalidatedLinks;
 ...
 * @param QueryStruct
 */
void OfflineBuild::stage7_bruteForce(QueryStructOfflineBuild& QueryStruct) {
    std::chrono::high_resolution_clock::time_point tStart_stage = std::chrono::high_resolution_clock::now();
    unsigned long long int const dStart_stage = get_distanceComputationCount();
