 ...
 * @param queryStruct
 */
void OnlineSearch::stage1(QueryStructOnlineSearch& queryStruct) {
    ...
    if (!coarserLayerNeighborsList.empty()) {
        ...
        if (queryStruct.flag_isCoarsePivot) {
            ...
            // if query is its own parent, then get all pivots as potential neighbors
            if (parentIndices.empty()) {
                ...
                activeCoarsePivotsList.insert(topLayerPivotIndices.begin(), topLayerPivotIndices.end());
            } else {
