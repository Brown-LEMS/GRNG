...
namespace OnlineSearch_stage6 {
bool checkLinkForInterference(QueryStructOnlineSearch& queryStruct, unsigned int const activeChildIndex, int const finerLayerIndex,
                              std::shared_ptr<PivotLayerHierarchy> const& pivotLayers, std::shared_ptr<SearchSparseMatrix> const& sparseMatrix);
...
void collectCandidateCoarseIndices(unsigned int const queryIndex, int const layerIndex, std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
                                   std::shared_ptr<SearchSparseMatrix> const& sparseMatrix,
...
bool dmaxCheck(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance, float const activeChildLinkDistance,
               ...
               std::shared_ptr<SearchSparseMatrix> const& sparseMatrix, RankOrderedList const& candidateInterferingPivots,
...
bool checkLinkForInterference(QueryStructOnlineSearch& queryStruct, unsigned int const activeChildIndex, int const finerLayerIndex,
                              std::shared_ptr<PivotLayerHierarchy> const& pivotLayers, std::shared_ptr<SearchSparseMatrix> const& sparseMatrix,
                              SearchSparseMatrixLocal& localSparseMatrix);
void collectCandidateCoarseIndices(unsigned int const queryIndex, int const layerIndex, std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
                                   tsl::sparse_set<unsigned int> const& activeInterferingPivotsList, RankOrderedList& rankedCandidateList,
                                   std::shared_ptr<SearchSparseMatrix> const& sparseMatrix, SearchSparseMatrixLocal& localSparseMatrix);
bool dmaxCheck(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance, float const activeChildLinkDistance,
               int const layerIndex, int const finerLayerIndex, std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
...
void collectCandidateFineIndices(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
                                 ...
                                 tsl::sparse_set<unsigned int>& fineInterferingPivotsList, std::shared_ptr<SearchSparseMatrix> const& sparseMatrix,
                                 SearchSparseMatrixLocal& localSparseMatrix);
bool assistedChildCheck(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
                        float const activeChildLinkDistance, unsigned int const coarseInterferingPivot, unsigned int const fineInterferingPivot,
                        std::shared_ptr<SearchSparseMatrix> const& sparseMatrix, SearchSparseMatrixLocal& localSparseMatrix);
 ...
 * @param queryStruct
 */
void OnlineSearch::stage6(QueryStructOnlineSearch& queryStruct) {
    ...
    for (it1 = activeChildrenList.begin(); it1 != activeChildrenList.end(); it1++) {
        ...
        bool flag_interference =
            OnlineSearch_stage6::checkLinkForInterference(queryStruct, activeChildIndex, finerLayerIndex, _pivotLayers, _sparseMatrix);
 ...
 * @return false
 */
bool OnlineSearch_stage6::checkLinkForInterference(QueryStructOnlineSearch& queryStruct, unsigned int const activeChildIndex,
    ...
    float const distance12 = sparseMatrix->getDistance(queryIndex, activeChildIndex);
    ...
    for (int layerIndex = 0; layerIndex < finerLayerIndex; layerIndex++) {
        ...
        if (layerIndex == 0) {
            ...
            collectCandidateCoarseIndices(queryIndex, layerIndex, pivotLayers, sparseMatrix, interferingPivotsMap[layerIndex - 1],
                                          candidateInterferingPivots);
        }
        ...
        flag_interference = dmaxCheck(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance, layerIndex, finerLayerIndex,
                                      pivotLayers, sparseMatrix, candidateInterferingPivots, interferingPivotsMap[layerIndex]);
    ...
    collectCandidateFineIndices(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance, finerLayerIndex, pivotLayers, sparseMatrix,
                                interferingPivotsMap[finerLayerIndex - 1], fineInterferingPivotsList);

    //------------------------------------------------------------------------------
    // Finally, brute force check for interference
    //------------------------------------------------------------------------------
    flag_interference = bruteForceInterferenceCheck(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance,
                                                    fineInterferingPivotsList, pivotLayers, sparseMatrix);
 ...
 */
void OnlineSearch_stage6::collectCandidateCoarseIndices(unsigned int const queryIndex, int const layerIndex,
                                                        std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
                                                        std::shared_ptr<SearchSparseMatrix> const& sparseMatrix,
    ...
    for (it1 = activeInterferingPivotsList.begin(); it1 != activeInterferingPivotsList.end(); it1++) {
        ...
        for (it2 = activeIndexChildrenList.begin(); it2 != activeIndexChildrenList.end(); it2++) {
            ...
            if (flag_add) {
                float const distance = sparseMatrix->getDistance(queryIndex, childIndex);
 ...
 * @return false
 */
bool OnlineSearch_stage6::dmaxCheck(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
    ...
    for (it1 = candidateInterferingPivots.list.begin(); it1 != candidateInterferingPivots.list.end(); it1++) {
        ...
        std::pair<bool, float> distance23_pair = sparseMatrix->getDistanceIfAvailable(activeChildIndex, candidateInterferingPivotIndex);
 ...
 */
void OnlineSearch_stage6::collectCandidateFineIndices(unsigned int const queryIndex, unsigned int const activeChildIndex,
                                                      float const queryLinkDistance, float const activeChildLinkDistance, int const finerLayerIndex,
                                                      std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
                                                      std::shared_ptr<SearchSparseMatrix> const& sparseMatrix,
                                                      tsl::sparse_set<unsigned int> const& coarseInterferingPivotsList,
                                                      tsl::sparse_set<unsigned int>& fineInterferingPivotsList) {
    ...
    for (it1 = coarseInterferingPivotsList.begin(); it1 != coarseInterferingPivotsList.end(); it1++) {
        ...
        for (it2 = coarseInterferingChildrenList.begin(); it2 != coarseInterferingChildrenList.end(); it2++) {
            ...
            flag_add = assistedChildCheck(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance, coarseInterferingIndex,
                                          fineInterferingIndex, pivotLayers, sparseMatrix);
 ...
 */
bool OnlineSearch_stage6::assistedChildCheck(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
    ...
    std::pair<bool, float> distance13_pair = sparseMatrix->getDistanceIfAvailable(queryIndex, coarseInterferingPivot);
    std::pair<bool, float> distance34_pair = sparseMatrix->getDistanceIfAvailable(coarseInterferingPivot, fineInterferingPivot);
    std::pair<bool, float> distance23_pair = sparseMatrix->getDistanceIfAvailable(activeChildIndex, coarseInterferingPivot);
 ...
 */
bool OnlineSearch_stage6::bruteForceInterferenceCheck(unsigned int const queryIndex, unsigned int const activeChildIndex,
    ...
    for (it1 = interferingPivotsList.begin(); it1 != interferingPivotsList.end(); it1++) {
        ...
        float const distance13 = sparseMatrix->getDistance(queryIndex, interferingPivotIndex);
        if (distance13 < queryLinkDistance) {
            float const distance23 = sparseMatrix->getDistance(activeChildIndex, interferingPivotIndex);
 ...
 * @param queryStruct
 */
void OnlineSearch::stage6_multithreading(QueryStructOnlineSearch& queryStruct) {
    std::chrono::high_resolution_clock::time_point tStart_stage = std::chrono::high_resolution_clock::now();
    unsigned long long int const dStart_stage = get_distanceComputationCount();
...
#pragma omp for schedule(static)
        for (int i = 0; i < (int)activeChildrenListVector.size(); i++) {
            ...
            // exhaustive, top-down search for interference
            bool flag_interference = OnlineSearch_stage6::multithreading::checkLinkForInterference(queryStruct, activeChildIndex, finerLayerIndex,
                                                                                                   _pivotLayers, _sparseMatrix, localSparseMatrix);
            if (!flag_interference) {
...
/**
 * @brief exhaustive, top-down check of link (queryIndex,activeChildIndex) for interference
 *
 * @param queryStruct
 * @param activeChildIndex
 * @param finerLayerIndex
 ...
 * @return false
 */
bool OnlineSearch_stage6::multithreading::checkLinkForInterference(QueryStructOnlineSearch& queryStruct, unsigned int const activeChildIndex,
                                                                   int const finerLayerIndex, std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
                                                                   std::shared_ptr<SearchSparseMatrix> const& sparseMatrix,
                                                                   SearchSparseMatrixLocal& localSparseMatrix) {
    unsigned int const queryIndex = queryStruct.queryIndex;
    ...
    //------------------------------------------------------------------------------
    tsl::sparse_set<unsigned int> fineInterferingPivotsList;
    collectCandidateFineIndices(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance, finerLayerIndex, pivotLayers,
                                interferingPivotsMap[finerLayerIndex - 1], fineInterferingPivotsList, sparseMatrix, localSparseMatrix);
 ...
 * @param localSparseMatrix
 */
void OnlineSearch_stage6::multithreading::collectCandidateCoarseIndices(unsigned int const queryIndex, int const layerIndex,
                                                                        std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
                                                                        tsl::sparse_set<unsigned int> const& activeInterferingPivotsList,
                                                                        RankOrderedList& rankedCandidateList,
                                                                        std::shared_ptr<SearchSparseMatrix> const& sparseMatrix,
 ...
 * @return false
 */
bool OnlineSearch_stage6::multithreading::dmaxCheck(unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance,
                                                    float const activeChildLinkDistance, int const layerIndex, int const finerLayerIndex,
                                                    std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
 ...
 * @param localSparseMatrix
 */
void OnlineSearch_stage6::multithreading::collectCandidateFineIndices(
    unsigned int const queryIndex, unsigned int const activeChildIndex, float const queryLinkDistance, float const activeChildLinkDistance,
    int const finerLayerIndex, std::shared_ptr<PivotLayerHierarchy> const& pivotLayers,
    tsl::sparse_set<unsigned int> const& coarseInterferingPivotsList, tsl::sparse_set<unsigned int>& fineInterferingPivotsList,
    std::shared_ptr<SearchSparseMatrix> const& sparseMatrix, SearchSparseMatrixLocal& localSparseMatrix) {
    //
    fineInterferingPivotsList.clear();
    ...
    for (it1 = coarseInterferingPivotsList.begin(); it1 != coarseInterferingPivotsList.end(); it1++) {
        ...
        for (it2 = coarseInterferingChildrenList.begin(); it2 != coarseInterferingChildrenList.end(); it2++) {
            ...
            // assisted parent-child check
            flag_add = assistedChildCheck(queryIndex, activeChildIndex, queryLinkDistance, activeChildLinkDistance, coarseInterferingIndex,
                                          fineInterferingIndex, sparseMatrix, localSparseMatrix);
 ...
 * @return false
 */
bool OnlineSearch_stage6::multithreading::assistedChildCheck(unsigned int const queryIndex, unsigned int const activeChildIndex,
                                                             float const queryLinkDistance, float const activeChildLinkDistance,
 ...
 * @return false
 */
bool OnlineSearch_stage6::multithreading::bruteForceInterferenceCheck(unsigned int const queryIndex, unsigned int const activeChildIndex,
                                                                      float const queryLinkDistance, float const activeChildLinkDistance,
                                                                      tsl::sparse_set<unsigned int> const& interferingPivotsList,
                                                                      std::shared_ptr<SearchSparseMatrix> const& sparseMatrix,
                                                                      SearchSparseMatrixLocal& localSparseMatrix) {
 ...
 * @param queryStruct
 */
void OnlineSearch::stage6_bruteForce(QueryStructOnlineSearch& queryStruct) {
    unsigned int const queryIndex = queryStruct.queryIndex;
    float const queryRadius = queryStruct.queryRadius;
